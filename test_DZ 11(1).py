...
try:
    ...
    sleep(7)
    login.send_keys('onosamoe', Keys.ENTER)
    password = browser.find_element(By.CSS_SELECTOR, f'[name="ws-input_{date}"][type = "password"]')
    sleep(7)
    password.send_keys('Test123', Keys.ENTER)
    sleep(7)
    Pop = browser.find_element(By.ID, "NavigationPanels-Accordion__container")
    sleep(7)
    if Pop.is_displayed():
        print("Контекстное меню отображается")
    else:
        print("Контекстное меню не отображается!")
